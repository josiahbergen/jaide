...
def encode_instruction(node: InstructionNode, context: AssemblyContext) -> bytearray:
    ...
    if not context.linkable and node.mnemonic == INSTRUCTIONS.JMP and node.operands[0].mode == MODES.IMM:
        logger.fatal(f"jump to absolute address on line {node.line}. use --bios-mode to enable low-level functionality.", "binary.py:encode_instruction()")
    ...
    if fmt.reg_a is not None:
        reg_a = node.operands[fmt.reg_a].register # type: ignore

    if fmt.reg_b is not None:
        reg_b = node.operands[fmt.reg_b].register # type: ignore
