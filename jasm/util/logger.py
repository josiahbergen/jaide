...
class Logger:
    ...
    def verbose(self, message: str):
        ...
        if self.level >= self.log_level.VERBOSE:
            formatted = f"{f.LIGHTBLACK_EX}{message}{f.RESET}"
            print(formatted)
    ...
    def kill(self, message: str, scope: str):
        ...
        newline = "\n" if self.level >= self.log_level.INFO else ""
        formatted = f"{newline}{b.RED}{f.BLACK} {message} {b.RESET}{f.RESET} process killed at {scope} {f.RESET}"
        print(formatted)
    ...
    def warning(
        self, message: str, scope: str | None = None, choice: bool = False
    ) -> None:
