...
class IRNode:
    ...
    def get_size(self) -> int:
        ...
        logger.fatal(
            f"get_size() not implemented for {type(self).__name__} on line {self.line}",
            scope,
        )
...
class Operand(IRNode):
    ...
    def get_value(self) -> int:
        ...
        logger.fatal(
            f"get_value() not implemented for {type(self).__name__} on line {self.line}",
            scope,
        )
...
class InstructionNode(IRNode):
    ...
    def get_opcode(self) -> int:
        try:
            ...
            return OPCODE_MAP[
                (
                    self.mnemonic,
                    tuple[MODES, ...](operand.mode for operand in self.operands),
                )
            ]
        except KeyError:
            key = (
            ...
            logger.fatal(
                f"invalid opcode for {self.mnemonic.name} with key {key} (line {self.line})",
                "ir.py:InstructionNode.get_opcode()",
            )
    ...
    def get_size(self) -> int:
        ...
        if any(
            (
                operand.mode
                in [MODES.IMM, MODES.RELATIVE, MODES.OFF_POINTER, MODES.REL_POINTER]
...
class DataDirectiveNode(IRNode):
    ...
    def __str__(self) -> str:
        strs = [
            f"{d[1]}" if d[0] == self.Type.NUMBER else f'"{d[1]}"' for d in self.items
        ]
    ...
    def parse_number(self, string: str) -> int:
        ...
        if value < -32768 or value > 0xFFFF:
            logger.fatal(
                f"number {string} ({value}) out of range for a 16-bit value (line {self.line})",
                scope,
            )
    ...
    def parse_string(self, string: str) -> list[int]:
        ...
        logger.verbose(
            f'parse_bytes: got bytes of string "{string}": {", ".join([str(byte) for byte in bytes])}'
        )
        return bytes
    ...
    def get_size(self) -> int:
        logger.verbose(
            f"data directive: got size {len(self.data)} (raw: {', '.join([str(byte) for byte in self.data])})"
        )
    ...
    def encode(self) -> bytearray:
        ...
        logger.verbose(
            f"get_bytes: {' '.join([f'{byte:04X}' for byte in bits])[:100]}... ({len(bits)} bytes from {', '.join([f'{d[1]}' for d in self.items])})"
        )
        return bits
...
class MacroDefinitionNode(IRNode):
    ...
    def expand(self, args_to_insert: list[Operand], line_of_call: int) -> list[IRNode]:
        ...
        placeholder_to_value: dict[str, Operand] = {
            placeholder: args_to_insert[i]
            for i, placeholder in enumerate[str](self.placeholders)
        }
        logger.verbose(
            f"macro: argument map: {', '.join([f'{k}: {v}' for k, v in placeholder_to_value.items()])}"
        )
        ...
        for element in template:
            if isinstance(element, MacroCallNode):
                ...
                logger.fatal(
                    f"nested macro calls are not yet supported: {self.name} (line {line_of_call})",
                    scope,
                )
                ...
                logger.warning(
                    f"macro contains data directive: {self.name} why are you doing this? (line {line_of_call})",
                    scope,
                )
                ...
                for i, operand in enumerate[Operand](element.operands):
                    if isinstance(operand, MacroArgumentOperand):
                        logger.verbose(
                            f"macro: replacing macro argument {operand.placeholder} with {placeholder_to_value[operand.placeholder]}"
                        )
                        element.operands[i] = placeholder_to_value[operand.placeholder]

            else:
                logger.fatal(
                    f"macros can only contain instructions: {self.name} (line {line_of_call})",
                    scope,
                )
    ...
    def _mangle_label(
        self, template: list[IRNode], label: LabelNode, line_of_call: int
    ) -> None:
        ...
        references = 0
        instructions = [
            element for element in template if isinstance(element, InstructionNode)
        ]
        for instruction in instructions:
            labels_to_change = [
                op
                for op in instruction.operands
                if isinstance(op, LabelOperand) and op.name == old_name
            ]
            for to_change in labels_to_change:
                to_change.name = new_label
                references += 1

        logger.verbose(
            f"macro: mangled label {old_name} and {references} references to {new_label} on line {line_of_call}"
        )
...
class MacroCallNode(IRNode):
    def __init__(self, line: int, name: str, args: list[Operand]):
        ...
        self.args: list[Operand] = (
            args  # real values to substitute for the placeholders
        )

    @override
    def __str__(self) -> str:
        return (
            f"macro call: {self.name} with {', '.join([str(op) for op in self.args])}"
        )
